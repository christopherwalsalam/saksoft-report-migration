...
export const getKpiGroups = async ({ search, kpiLabel, page = 1, pageSize = 20 } = {}) => {
  ...
  const { data } = await axiosInstance.get('/api/kpi-groups', { params });
  return data;
};

/**
 * GET /api/kpi-groups/{groupId}/reports
 * Returns all reports in a KPI group with full detail
 */
export const getKpiGroupReports = async (groupId) => {
  const { data } = await axiosInstance.get(`/api/kpi-groups/${groupId}/reports`);
  return data.data ?? data;
};
